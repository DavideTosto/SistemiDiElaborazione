...
int ricercaValore(int arr[], int dim, int val, int i)
{
    ...
    if (arr[i] == val)
        return i;                               // Caso base: valore trovato
    return ricercaValore(arr, dim, val, i + 1); // Ricorsione
}

int main()
{
    int n, scelta, val, flagCambio = 1;
    int arr[DIM];

    printf("Ricevuti in ingresso gli elementi di un vettore di dimensione specificata dall’utente e \ndimensione massima DIM = 20 ");
    printf("\nSi // Stampa iniziale del vettore
        printf("Vettore iniziale: ");
        stampaVettore(arr, n);implementeranno le funzioni di ordinamento decrescente o ricerca\n");
    ...
    do
    {
        ...
        printf("Vuoi cambiare gli elementi del vettore? (Y/N)")
        scanf("%c", ) // modificare e implementare soluzione numeri con la virgola
